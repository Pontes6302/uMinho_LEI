...
float calcular_media_precos(Produto menu[], int tamanho) {
    float soma = 0.0;
...
int contem_ingredientes(char **ingredientes_produto, int num_ingredientes, char *ingrediente1, char *ingrediente2) {
    int encontrou_ingrediente1 = 0, encontrou_ingrediente2 = 0;
    for (int i = 0; i < num_ingredientes; i++) {
        if (strcmp(ingredientes_produto[i], ingrediente1) == 0) {
            encontrou_ingrediente1 = 1;
        }
        if (strcmp(ingredientes_produto[i], ingrediente2) == 0) {
            encontrou_ingrediente2 = 1;
        }
    }
    return encontrou_ingrediente1 && encontrou_ingrediente2;
}
...
int main(){
    qsort(menu, sizeof(menu)/sizeof(menu[0]), sizeof(Produto), cmp);
    for (int i = 1; i < sizeof(menu)/sizeof(menu[0]); i++){
        printf("%da: %d %s %.2f\n", i,  menu[i].id, menu[i].nome, menu[i].preco);
    }
}
/*
int main() {
    ...
    float media_precos = calcular_media_precos(menu, tamanho);

    // Contar produtos que contêm "Açúcar" e "Fermento" e têm preço acima da média
    int contador = 0;
    for (int i = 0; i < tamanho; i++) {
        if (menu[i].preco > media_precos && contem_ingredientes(menu[i].ingredientes, 5,  "Açúcar", "Fermento")) {
            contador++;
        }
    }

    // Imprimir o resultado
    printf("Número de produtos com ingredientes {\"Açúcar\", \"Fermento\"} e preço acima da média: %d\n", contador);
