# 🎓 uMinho - Engenharia Informática (LEI)

Repositório centralizador de projetos e materiais de estudo da Licenciatura em Engenharia Informática na **Universidade do Minho**.

---

## 📂 Estrutura do Curso

### [1º Ano](./1%20Ano)

**1º Semestre**
*   **Álgebra** - Álgebra Linear
*   **Cálculo** - Cálculo
*   **Informação e Direito** - Informação e Direito
*   **LI1** - Laboratórios de Informática I
*   **PF** - Programação Funcional
*   **TMD** - Tópicos de Matemática Discreta

**2º Semestre**
*   **Análise** - Análise Matemática
*   **EPTN** - Elementos de Probabilidades e Teoria de Números
*   **LI2** - Laboratórios de Informática II
*   **Lógica** - Lógica Computacional
*   **PI** - Programação Imperativa
*   **SC** - Sistemas de Computação

---

### [2º Ano](./2%20Ano)

**1º Semestre**
*   **AlgC** - Algoritmos e Complexidade
*   **ArqC** - Arquitetura de Computadores
*   **BD** - Bases de Dados
*   **EA** - Estatística Aplicada
*   **FCD** - Fundamentos da Comunicação de Dados
*   **LI3** - Laboratórios de Informática III

---

## 🚀 Como utilizar

Se quiseres explorar algum projeto localmente:

```bash
git clone --recursive https://github.com
```

---

## 👤 Autor

**Sívio Pontes** 
